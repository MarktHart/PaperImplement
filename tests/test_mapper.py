...
@pytest.fixture
def model(hf_model):
    return Bert(L=hf_model.config.num_hidden_layers,
                H=hf_model.config.hidden_size,
                A=hf_model.config.num_attention_heads,
                dict_size=hf_model.config.vocab_size)
...
def test_map_model_extra_lin_on_bert(caplog, hf_model, model, dummy_inputs, clone_inputs):
    ...
    with pytest.raises(ValueError):
        map_model(original=hf_model, clone=model, original_input=dummy_inputs, clone_input=clone_inputs)
