...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.dropout(self.layernorm(word_encoding + segment_encoding + positional_encoding))
...
class Layer(nn.Module):
    ...
    def forward(self, attention_in: tuple[torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        x, mask = attention_in
        return self.out(self.attention(x, mask=mask)), mask
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, **kwargs)-> torch.Tensor:
        return self.norm(x + self.drop(self.block(x, **kwargs)))
